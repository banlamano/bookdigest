...
async function fixCovers() {
  console.log('🎨 Detecting and fixing placeholder covers...');
  ...
  for (const dbBook of allBooks) {
    const info = await getLength(dbBook.coverImage);
    if (info.len === 9103 || info.len === 9 || info.len === 43 || info.len === 0 || !dbBook.coverImage.startsWith('http')) {
      ...
      try {
        ...
        console.log(`✅ Fixed: ${dbBook.title} (was len ${info.len})`);
        fixed++;
      } catch (error) {
        console.error(`❌ Failed: ${dbBook.title}`, error.message);
      }
    }
  }
  
  console.log(`\nDone! Fixed ${fixed} covers.`);
