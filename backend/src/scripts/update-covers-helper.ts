...
export async function runCoverUpdate() {
  let updated = 0;
  let skipped = 0;
  let errors = 0;
  ...
  for (const book of coverUpdates) {
    try {
      ...
      // Skip if already has Google Books cover
      if (existing.coverImage?.includes('googleapis.com')) {
        skipped++;
        continue;
      }
  ...
  return { updated, skipped, errors, total: coverUpdates.length };
}
